    ...
    <AppShell title="">
      <div className="flex flex-col items-center px-6 pt-8 pb-8">
        <a
          ...
          style={{ width: 96, height: 96, aspectRatio: "1 / 1" }}
        >
          <img src={logoUrl} alt="Entendernos" style={{ width: "100%", height: "100%", objectFit: "contain", display: "block" }} />
        </a>
        <h1 className="mt-6 text-white text-4xl" style={{ fontFamily: "var(--font-display)", fontWeight: 700 }}>Entendernos</h1>
        <p className="mt-4 text-center text-white/85 max-w-xs leading-snug">
          Esta es una muestra digital. Nuestro objetivo es promover el encuentro físico y humano a través de un mazo de cartas lúdico.
        </p>

        <ul className="mt-8 w-full max-w-sm grid grid-cols-2 gap-4 px-2">
          {features.map(({ Icon, title, body }) => (
            <li key={title} className="flex flex-col items-center text-center gap-1.5 bg-white/10 rounded-2xl p-3 border-0 shadow-none">
        ...
        <div className="mt-10 w-full max-w-sm flex flex-col gap-3">
          ...
          <BuyDeckButton variant="outline" className="!border-white !text-white" />
        </div>
      </div>
    </AppShell>
  );
